...
* **🛡️ Stability, Recovery & Collaboration Hardening:**
  ...
  * **OTAcademy RME-Compliant Stacking & Auto Town Detection:** Restored authentic Remere's Map Editor / OTAcademy stacking mechanics in `Tile::addItem`. Items, furniture (tables, counters), depots, and doodads can now be freely stacked on the same tile up to the Tibia protocol stack limit (10 items), while preserving proper floorchange replacement and intelligent nearest-town assignment for depots.
