...
#define String(key, dflt)                             \
	...
	Float(MINIMAP_SCROLL_SPEED, 2.0f);
	...
	Int(ICON_BACKGROUND, 88); // Standardmäßig auf Grau/Dunkel für RPG-Look
	Int(HARD_REFRESH_RATE, 4); // Ca. 250 FPS theoretisches Limit für UI-Refresh
	...
	Int(MINIMAP_DOCK_STYLE, 0);
	Float(MINIMAP_OPACITY, 0.98f);
	Int(MINIMAP_VIEW_BOX, 1);
	String(MINIMAP_EXPORT_DIR, "");
	String(TILESET_EXPORT_DIR, "");

	Int(CURSOR_RED, 255); // Goldener Cursor für Fantasy-Feeling
	Int(CURSOR_GREEN, 215);
	Int(CURSOR_BLUE, 0);
	Int(CURSOR_ALPHA, 128);
	Int(CURSOR_ALT_RED, 0); // Blau für sekundäre Aktionen
	Int(CURSOR_ALT_GREEN, 191); // Fix für log: CURSOR_ALT_ALT_GREEN Tippfehler korrigiert
	Int(CURSOR_ALT_BLUE, 255);
