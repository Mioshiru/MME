...
void LiveClient::parseHello(NetworkMessage& message) {
	...
	if (mapEditor) {
		// Close any canvas tabs still referencing this mapEditor before
		// deleting it, to prevent use-after-free during the reconnect flow.
...
void LiveClient::checkAndApplyHostSettings() {
	...
	// Always auto-apply the host's view settings — no dialog, no stored preference.
	// Remote clients simply mirror the host's active workspace state on join.
...
void LiveClient::applyPendingWorldPalettes() {
	...
	auto addBrushSafe = [](TilesetCategory* cat, Brush* b) {
		if (!cat || !b) return;
		for (Brush* existing : cat->brushlist) {
