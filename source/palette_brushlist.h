...
public:
	BrushBoxInterface(const std::vector<Brush*>& brushes) :
		loaded(false), all_brushes(brushes) {
	...
	virtual void Filter(const wxString& query) = 0;

protected:
	bool loaded;
	std::vector<Brush*> all_brushes;
	std::vector<Brush*> visible_brushes;
};

class BrushListBox : public wxVListBox, public BrushBoxInterface {
public:
	BrushListBox(wxWindow* parent, const std::vector<Brush*>& brushes);
	~BrushListBox();
...
public:
	BrushIconBox(wxWindow* parent, const std::vector<Brush*>& brushes, RenderSize rsz);
	~BrushIconBox();
...
public:
	...
	void Filter(const wxString& query);
	void AssignTileset(const TilesetCategory* tileset);
	void AssignBrushes(const std::vector<Brush*>& brushes);
