...
class PrefabBrush : public Brush {
public:
	PrefabBrush(const std::string& name, const std::string& data, const std::string& iconName);
	PrefabBrush(const std::string& name, const std::string& data);
...
public:
	PrefabCreatorBrush();
	virtual ~PrefabCreatorBrush();

	bool load(pugi::xml_node node, wxArrayString& warnings) override { return true; }

	void draw(BaseMap* map, Tile* tile, void* parameter = nullptr) override;
	void undraw(BaseMap* map, Tile* tile) override;
	bool canDraw(BaseMap* map, const Position& position) const override { return true; }

	std::string getName() const override { return "Prefab Creator Brush"; }
	int getLookID() const override { return EDITOR_SPRITE_PREFAB; }
	bool canDrag() const override { return true; }
	bool oneSizeFitsAll() const override { return true; }
	bool isSelectionOnly() const override { return true; }

	void markPosition(const Position& pos, bool select);
	void clear() { selected_tiles.clear(); }
	const std::set<Position>& getSelectedTiles() const { return selected_tiles; }

protected:
	std::set<Position> selected_tiles;
};
