...
MapPropertiesWindow::MapPropertiesWindow(wxWindow* parent, MapTab* view, Editor& editor) :
	...
	// Title Header
	wxStaticText* header_text = new wxStaticText(this, wxID_ANY, "Map Properties");
	wxFont header_font = header_text->GetFont();
	header_font.SetPointSize(14);
	header_font.SetWeight(wxFONTWEIGHT_BOLD);
	header_text->SetFont(header_font);
	header_text->SetForegroundColour(wxColor(180, 140, 50)); // Gold
	topsizer->Add(header_text, 0, wxALIGN_CENTER_HORIZONTAL | wxTOP | wxBOTTOM, 15);

	wxStaticBoxSizer* general_box = new wxStaticBoxSizer(wxVERTICAL, this, "General Information");
	...
	wxStaticBoxSizer* details_box = new wxStaticBoxSizer(wxVERTICAL, this, "Dimensions & External Files");
	wxFlexGridSizer* details_grid = newd wxFlexGridSizer(2, 10, 10);
	details_grid->AddGrowableCol(1);
	...
	button_sizer->Add(newd wxButton(this, wxID_OK, "OK"), wxSizerFlags(0).Border(wxRIGHT, 10));
	button_sizer->Add(newd wxButton(this, wxID_CANCEL, "Cancel"), wxSizerFlags(0));
	topsizer->Add(button_sizer, wxSizerFlags(0).CenterHorizontal().Border(wxBOTTOM, 15));
...
EditTownsDialog::EditTownsDialog(wxWindow* parent, Editor& editor) :
	...
	wxButton* genBtn = new wxButton(this, wxID_ANY, "Dice", wxDefaultPosition, wxSize(32, 24));
	genBtn->SetToolTip("Generate Random Town Name");
	genBtn->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
		static std::vector<std::string> pre = {"Oak", "Silver", "Iron", "Raven", "Mist", "Storm", "Eldor"};
		static std::vector<std::string> post = {"haven", "moon", "forge", "loft", "wood", "wind", "ia"};
		this->name_field->SetValue(wxstr(pre[rand()%pre.size()] + post[rand()%post.size()]));
	});
	tmpsizer->Add(genBtn, 0, wxLEFT | wxBOTTOM, 5);
