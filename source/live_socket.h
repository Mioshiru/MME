...
class Tile;
class BinaryNode;
class MemoryNodeFileWriteHandle;
...
public:
	...
	void receiveTile(BinaryNode* node, MapEditor& editor, Action* action, const Position* position);
	void sendTile(MemoryNodeFileWriteHandle& writer, Tile* tile, const Position* position);

	// read / write types
	Tile* readTile(BinaryNode* node, MapEditor& editor, const Position* position);
