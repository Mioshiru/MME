...
#include "ui_toolbar.h" // Für RME_UI::UIToolbar
#include "gui_ids.h"
...
class GUI : public IGuiLayout, public IGuiAssets {
...
public:
	...
	void ToggleMinimapHUD();
	uint32_t GetHoveredPaletteItemID() const;
	Item* GetSelectedPropertiesItem();
	void OpenProperties(Item* item);
	void CloseProperties();
	uint32_t m_palette_hover_id = 0;
	Item* m_imgui_properties_item = nullptr;
	...
	void SavePrefabFromCreator();
	RME_Rendering::RenderBackend* GetRenderBackend();
...
public:
	...
	std::string m_graphicsErrorLog;
	wxWindow* in_game_preview;
	BrushToolPanel* tools_panel;
	BrushSizePanel* size_panel;
	DCButton* gem; // The small gem in the lower-right corner
	SearchResultWindow* search_result_window;
	GraphicManager gfx;
	RME::Core::AsyncLoader* async_loader;
	bool m_vulkan_failed = false;
	std::unique_ptr<RME_Rendering::RenderBackend> render_backend;
	...
	FlagBrush* pvp_brush;
	PrefabCreatorBrush* prefab_creator_brush;
