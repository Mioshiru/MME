...
namespace LuaAPI {
	...
	// BSP Node for dungeon generation (defined at namespace scope to avoid
	// MSVC calling-convention issues with types defined inside lambdas)
	struct BSPNode {
		int x, y, w, h;
	...
	void registerAlgo(sol::state& lua) {
		...
		algoTable.set_function("generateDungeon", [](int width, int height, sol::optional<sol::table> options, sol::this_state s) -> sol::table {
			...
			std::function<BSPNode*(int, int, int, int, int)> split =
				[&](int x, int y, int w, int h, int depth) -> BSPNode* {
			...
			// Cleanup BSP
			std::function<void(BSPNode*)> deleteBSP;
			deleteBSP = [&](BSPNode* node) {
				if (node) {
					deleteBSP(node->left);
