...
#include "../house.h"
	...
	static bool hasMap() {
		MapEditor* editor = g_gui.GetCurrentEditor();
	...
	void registerApp(sol::state& lua) {
		...
		lua.new_usertype<MapEditor>(
			"MapEditor",
			sol::no_constructor,

			// Undo/Redo functions
			"undo", [](MapEditor* editor) {
			if (editor && editor->actionQueue && editor->actionQueue->canUndo()) {
				editor->actionQueue->undo();
				g_gui.RefreshView();
			} },
			"redo", [](MapEditor* editor) {
			if (editor && editor->actionQueue && editor->actionQueue->canRedo()) {
				editor->actionQueue->redo();
				g_gui.RefreshView();
			} },
			"canUndo", [](MapEditor* editor) -> bool { return editor && editor->actionQueue && editor->actionQueue->canUndo(); },
			"canRedo", [](MapEditor* editor) -> bool { return editor && editor->actionQueue && editor->actionQueue->canRedo(); },

			// History info
			"historyIndex", sol::property([](MapEditor* editor) -> int {
				if (editor && editor->actionQueue) {
					return (int)editor->actionQueue->getCurrentIndex();
				}
				return 0;
			}),
			"historySize", sol::property([](MapEditor* editor) -> int {
				if (editor && editor->actionQueue) {
			...
			"getHistory", [](MapEditor* editor, sol::this_state ts) -> sol::table {
			sol::state_view lua(ts);
			...
			"goToHistory", [](MapEditor* editor, int targetIndex) {
