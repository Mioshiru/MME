...
void GUI::OnWelcomeDialogAction(wxCommandEvent &event) {
  ...
  if (event.GetId() == wxID_NEW) {
    ...
    wxString basePathStr = GUI::GetExecDirectory();
    std::filesystem::path rootDir(basePathStr.wc_str());
    ...
    for (size_t i = 1; i <= 50; ++i) {
      ...
      if (!slot_occupied) {
        target_slot_dir = wxString(slotDir.wstring().c_str());
        break;
      }
    }

    if (target_slot_dir.empty()) {
      target_slot_dir = basePathStr + "Saves/Slot 1";
    }
...
void GUI::SelectBrushInternal(Brush *brush) {
  ...
  // Automatic 1x1 brush size reset for Wall, Door, Window, Monster/NPC, RAW, HouseExit, Waypoint
  if (brush->isWall() || brush->isDoor() || brush->isCreature() || brush->isRaw() ||
