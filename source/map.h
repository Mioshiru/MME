...
public:
	...
	using BaseMap::createTileL; // Verhindert Name-Hiding der Position-Überladung
	TileLocation* createTileL(int x, int y, int z);
    
	void beginBatchChange();
	void endBatchChange();
	bool isBatching() const { return batch_depth > 0; }
	std::vector<uint8_t> createRegionSnapshot(int x, int y, int width, int height);

	void clearDirtyFlags();
	void restoreRegionSnapshot(int x, int y, const std::vector<uint8_t>& snapshot);

	void exportOTClient(const std::string& directory);
...
protected:
	int batch_depth = 0;
	std::unique_ptr<RME::Core::ECS::ECSManager> m_ecs;
};
