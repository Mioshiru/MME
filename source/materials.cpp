...
const MaterialsExtensionList &Materials::getExtensions() { return extensions; }
...
bool Materials::loadMaterials(const FileName &identifier, wxString &error,
                              wxArrayString &warnings) {
  pugi::xml_document doc;
  pugi::xml_parse_result result =
      doc.load_file(identifier.GetFullPath().mb_str());
  if (!result) {
    warnings.push_back("Could not open " + identifier.GetFullName() +
...
bool Materials::loadExtensions(FileName directoryName, wxString &error,
                               wxArrayString &warnings) {
  ...
  do {
    ...
    if (extensionName.empty() || extensionAuthor.empty() ||
        extensionDescription.empty()) {
      warnings.push_back(
          filename +
          ": Couldn't read extension attributes (name, author, description).");
      continue;
    }

    std::string extensionUrl = extensionNode.attribute("url").as_string();
    extensionUrl.erase(
        std::remove(extensionUrl.begin(), extensionUrl.end(), '\''));

    std::string extensionAuthorLink =
        extensionNode.attribute("authorurl").as_string();
    extensionAuthorLink.erase(std::remove(extensionAuthorLink.begin(),
                                          extensionAuthorLink.end(), '\''));

    MaterialsExtension *materialExtension = newd MaterialsExtension(
        extensionName, extensionAuthor, extensionDescription);
    materialExtension->url = extensionUrl;
    materialExtension->author_url = extensionAuthorLink;
    materialExtension->filename = std::string(filename.mb_str());
    ...
    if ((attribute = extensionNode.attribute("client"))) {
      ...
      while (position != std::string::npos) {
        clientVersions.push_back(extensionClientString.substr(
      ...
      std::sort(materialExtension->version_list.begin(),
                materialExtension->version_list.end(),
                VersionComparisonPredicate);

      auto duplicate = std::unique(materialExtension->version_list.begin(),
                                   materialExtension->version_list.end());
      while (duplicate != materialExtension->version_list.end()) {
        materialExtension->version_list.erase(duplicate);
        duplicate = std::unique(materialExtension->version_list.begin(),
                                materialExtension->version_list.end());
      }
    } else {
      warnings.push_back(filename +
                         ": Extension is not available for any version.");
    }
...
bool Materials::unserializeMaterials(const FileName &filename,
                                     pugi::xml_node node, wxString &error,
                                     wxArrayString &warnings) {
  wxString warning;
  pugi::xml_attribute attribute;
  for (pugi::xml_node childNode = node.first_child(); childNode;
    ...
    if (childName == "include") {
      ...
      if (!loadMaterials(includeName, subError, warnings)) {
        warnings.push_back("Error while loading file \"" +
                           includeName.GetFullName() + "\": " + subError);
      }
...
void Materials::rebuildFavorites() {
  ...
  // Collect unique real brushes while preserving exact user order
  std::vector<Brush *> ordered_favs;
  ...
  // Scan catFav first to respect the exact list and order
  for (Brush *b : catFav->brushlist) {
    ...
    } else if (b->isDoodad() || b->isTable() || b->isCarpet() ||
               wxstr(b->getName()).Lower().Contains("ladder")) {
  ...
  // Also check other categories in case items were added elsewhere
  ...
  if (!terrain_favs.empty()) {
    ...
    catFav->brushlist.insert(catFav->brushlist.end(), terrain_favs.begin(),
                             terrain_favs.end());
  }
  if (!wall_favs.empty()) {
    catFav->brushlist.push_back(&sep_walls);
    catFav->brushlist.insert(catFav->brushlist.end(), wall_favs.begin(),
                             wall_favs.end());
  }
  if (!doodad_favs.empty()) {
    catFav->brushlist.push_back(&sep_doodads);
    catFav->brushlist.insert(catFav->brushlist.end(), doodad_favs.begin(),
                             doodad_favs.end());
  }
  if (!item_favs.empty()) {
    catFav->brushlist.push_back(&sep_items);
    catFav->brushlist.insert(catFav->brushlist.end(), item_favs.begin(),
                             item_favs.end());
  }
  if (!monster_favs.empty()) {
    catFav->brushlist.push_back(&sep_monsters);
    catFav->brushlist.insert(catFav->brushlist.end(), monster_favs.begin(),
                             monster_favs.end());
  }
  if (!npc_favs.empty()) {
    catFav->brushlist.push_back(&sep_npcs);
    catFav->brushlist.insert(catFav->brushlist.end(), npc_favs.begin(),
                             npc_favs.end());
  }
  if (!other_favs.empty()) {
    catFav->brushlist.push_back(&sep_other);
    catFav->brushlist.insert(catFav->brushlist.end(), other_favs.begin(),
                             other_favs.end());
  }
}
...
void Materials::saveFavorites() {
    ...
    for (Brush *b : catFav->brushlist) {
      if (b && !b->isSeparator() && !b->getName().empty()) {
        if (saved_names.find(b->getName()) == saved_names.end()) {
          ...
          uint32_t tagColor = getFavoriteTagColor(b->getName());
          if (tagColor != 0) {
            char tagHex[16];
            snprintf(tagHex, sizeof(tagHex), "#%06X", tagColor & 0xFFFFFF);
            item.append_attribute("tag_color") = tagHex;
          }
        }
      }
    }
  ...
  // Also save to Saves folder if present
  wxFileName savesFn("Saves/Slot 1/favorites.xml");
  if (wxDirExists("Saves/Slot 1")) {
    doc.save_file(savesFn.GetFullPath().mb_str());
  }
...
bool Materials::addFavoriteBrush(Brush* brush) {
  if (!brush || brush->isSeparator() || brush->getName().empty()) return false;
  Tileset* favs = tilesets["Favorites"];
...
void Materials::loadFavorites() {
  std::vector<wxFileName> candidate_paths;
  candidate_paths.push_back(GetFavoritesFilePath());
  candidate_paths.push_back(wxFileName("Saves/Slot 1/favorites.xml"));
  candidate_paths.push_back(wxFileName("Saves/favorites.xml"));
  candidate_paths.push_back(wxFileName(g_gui.GetDataDirectory(), "favorites.xml"));

  ClientVersionID verId = g_gui.GetCurrentVersionID();
  if (verId != CLIENT_VERSION_NONE) {
    if (ClientVersion *cv = ClientVersion::get(verId)) {
      candidate_paths.push_back(wxFileName(cv->getDataPath().GetPath(), "favorites.xml"));
    }
  }
  candidate_paths.push_back(wxFileName(wxStandardPaths::Get().GetUserDataDir(), "favorites.xml"));
  ...
  for (const wxFileName& fn : candidate_paths) {
    ...
    for (pugi::xml_node item = root.child("brush"); item; item = item.next_sibling("brush")) {
      ...
      if (brush_name && strlen(brush_name) > 0) {
        ...
        if (b && !b->isSeparator()) {
          if (!catFav->containsBrush(b)) {
            catFav->brushlist.push_back(b);
            loaded_any = true;
          }
          const char *tagStr = item.attribute("tag_color").as_string("");
          if (tagStr && strlen(tagStr) > 0) {
            unsigned int colorVal = 0;
            if (tagStr[0] == '#') {
              sscanf(tagStr + 1, "%x", &colorVal);
            } else {
              sscanf(tagStr, "%x", &colorVal);
            }
            if (colorVal != 0) {
              setFavoriteTagColor(brush_name, colorVal);
            }
          }
        }
      }
...
void Materials::addToTileset(std::string tilesetName, int itemId,
...
bool Materials::isInTileset(Item *item, std::string tilesetName) const {
  ...
  return it.id != 0 && (isInTileset(it.brush, tilesetName) ||
