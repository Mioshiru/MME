...
MapEditor::MapEditor(CopyBuffer& copybuffer, const FileName& fn) :
	live_server(nullptr),
	live_client(nullptr),
	actionQueue(newd ActionQueue(*this)),
	selection(*this),
...
MapEditor::MapEditor(CopyBuffer& copybuffer, LiveClient* client) :
	live_server(nullptr),
	live_client(client),
...
void MapEditor::addBatch(BatchAction* action, int stacking_delay) {
	actionQueue->addBatch(action, stacking_delay);
	g_gui.UpdateMenus();
}

void MapEditor::addAction(Action* action, int stacking_delay) {
...
bool MapEditor::exportMiniMap(FileName filename, int floor /*= GROUND_LAYER*/, bool displaydialog) {
	return map.exportMinimap(filename, floor, displaydialog);
}

bool MapEditor::exportSelectionAsMiniMap(FileName directory, wxString fileName) {
...
bool MapEditor::importMap(FileName filename, int import_x_offset, int import_y_offset, ImportType house_import_type, ImportType spawn_import_type) {
	selection.clear();
	actionQueue->clear();
...
void MapEditor::destroySelection() {
	if (selection.size() == 0) {
		...
		if (g_settings.getInteger(Config::USE_AUTOMAGIC) && ground_deleted) {
			// Remove duplicates
			tilestoborder.sort();
...
void MapEditor::drawInternal(const PositionVector& tilestodraw, bool alt, bool dodraw) {
	Brush* brush = g_gui.GetCurrentBrush();
	if (!brush) {
		return;
	}
...
bool MapEditor::IsLiveClient() const {
	return live_client != nullptr;
}

bool MapEditor::IsLiveServer() const {
	return live_server != nullptr;
}

bool MapEditor::IsLive() const {
	return IsLiveClient() || IsLiveServer();
}

bool MapEditor::IsLocal() const {
	return !IsLive();
}

LiveClient* MapEditor::GetLiveClient() const {
	return live_client;
}

LiveServer* MapEditor::GetLiveServer() const {
	return live_server;
}

LiveSocket& MapEditor::GetLive() const {
	if (live_server) {
		return *live_server;
	}
	return *live_client;
...
LiveServer* MapEditor::StartLiveServer() {
	ASSERT(IsLocal());
...
void MapEditor::CloseLiveServer() {
	ASSERT(IsLive());
	if (live_client) {
		live_client->close();
...
void MapEditor::QueryNode(int ndx, int ndy, bool underground) {
	ASSERT(live_client);
	live_client->queryNode(ndx, ndy, underground);
}

void MapEditor::SendNodeRequests() {
	if (live_client) {
		live_client->sendNodeRequests();
	}
}

void MapEditor::setDeferBorders(bool defer) {
	defer_borders = defer;
	if (!defer) {
		commitBorders();
	}
...
void MapEditor::commitBorders() {
	if (pending_borders.empty()) {
		return;
	}
	std::sort(pending_borders.begin(), pending_borders.end());
	pending_borders.erase(std::unique(pending_borders.begin(), pending_borders.end()), pending_borders.end());

	BatchAction* batch = actionQueue->createBatch(ACTION_DRAW);
	Action* action = actionQueue->createAction(batch);

	for (const auto& pos : pending_borders) {
		TileLocation* location = map.createTileL(pos);
		Tile* tile = location->get();
		if (tile) {
			Tile* new_tile = tile->deepCopy(map);
