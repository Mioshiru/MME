...
Creature::Creature(CreatureType* ctype) :
	direction(SOUTH), spawntime(0), loot_rate(0), saved(false), selected(false) {
	if (ctype) {
...
Creature::Creature(std::string ctype_name) :
	type_name(ctype_name), direction(SOUTH), spawntime(0), loot_rate(0), saved(false), selected(false) {
	////
}
