...
// OTMM Item serialization - uses standard OTBM interface for now
// The OTMM-specific methods are commented out in item.h as this format is partially supported

// Use map.createTile instead of map.allocator for tile creation
Tile* map_allocator_wrapper(BaseMap& map, const Position& pos) {
	return map.createTile(pos.x, pos.y, pos.z);
}
	uint16_t _id;
				...
				case OTMM_SPAWN_DATA: {
					...
					if (spawnNode) {
						do {
							...
							uint8_t spawn_z;
							uint32_t radius;
							if (!spawnNode->getU16(spawn_x) || !spawnNode->getU16(spawn_y) || !spawnNode->getU8(spawn_z)) {
							...
							if (creatureNode) {
								do {
									...
									uint8_t creature_z;
									if (!creatureNode->getU16(creature_x) || !creatureNode->getU16(creature_y) || !creatureNode->getU8(creature_z)) {
									...
									if (creature_tile->spawn == nullptr) {
										// No spawn, create a newd one (this happens if the radius of the spawn has been decreased due to g_settings)
...
bool IOMapOTMM::saveMap(Map& map, const FileName& identifier, bool showdialog) {
	DiskNodeFileWriteHandle f(std::string(identifier.GetFullPath().mb_str(wxConvUTF8)), "OTMM");
