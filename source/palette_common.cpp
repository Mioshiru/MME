...
void BrushSizePanel::LoadAllContents() {
	...
	wxWrapSizer* size_sizer = newd wxWrapSizer(wxHORIZONTAL);
	RenderSize render_size = large_icons ? RENDER_SIZE_32x32 : RENDER_SIZE_16x16;

	size_sizer->Add(brushshapeSquareButton = newd DCButton(this, PALETTE_BRUSHSHAPE_SQUARE, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_9x9));
	brushshapeSquareButton->SetToolTip("Square brush");

	size_sizer->Add(brushshapeCircleButton = newd DCButton(this, PALETTE_BRUSHSHAPE_CIRCLE, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_CD_9x9));
	brushshapeCircleButton->SetToolTip("Circle brush");
	brushshapeSquareButton->SetValue(true);

	size_sizer->AddSpacer(large_icons ? 36 : 18);

	size_sizer->Add(brushsize0Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_0, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_CD_1x1));
	brushsize0Button->SetToolTip("Brush size 1");
	brushsize0Button->SetValue(true);

	size_sizer->Add(brushsize1Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_1, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_3x3));
	brushsize1Button->SetToolTip("Brush size 2");

	size_sizer->Add(brushsize2Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_2, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_5x5));
	brushsize2Button->SetToolTip("Brush size 3");

	size_sizer->Add(brushsize4Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_4, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_7x7));
	brushsize4Button->SetToolTip("Brush size 5");

	size_sizer->Add(brushsize6Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_6, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_9x9));
	brushsize6Button->SetToolTip("Brush size 7");

	size_sizer->Add(brushsize8Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_8, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_15x15));
	brushsize8Button->SetToolTip("Brush size 9");

	size_sizer->Add(brushsize11Button = newd DCButton(this, PALETTE_TERRAIN_BRUSHSIZE_11, wxDefaultPosition, DC_BTN_TOGGLE, render_size, EDITOR_SPRITE_BRUSH_SD_19x19));
...
wxSize BrushSizePanel::GetMinSize() const {
	if (!loaded) return wxPanel::GetMinSize();
	
	int dock_dir = 0;
	if (g_gui.GetAuiManager()) {
		wxAuiPaneInfo& pane = g_gui.GetAuiManager()->GetPane(const_cast<BrushSizePanel*>(this));
		if (pane.IsOk() && !pane.IsFloating()) {
			dock_dir = pane.dock_direction;
		}
	}
	
	int btn_size = large_icons ? 32 : 16;
	if (dock_dir == wxAUI_DOCK_TOP || dock_dir == wxAUI_DOCK_BOTTOM) {
		int min_w = 9 * btn_size + (large_icons ? 36 : 18) + 16;
		int min_h = btn_size + 8;
		return wxSize(min_w, min_h);
	} else {
		int min_w = 3 * btn_size + 16; 
		int min_h = 3 * btn_size + 16;
		return wxSize(min_w, min_h);
	}
}
...
EVT_TOGGLEBUTTON(PALETTE_TERRAIN_PREFAB_CREATOR_TOOL, BrushToolPanel::OnClickPrefabCreatorButton)
...
void BrushToolPanel::LoadAllContents() {
	...
	size_sizer->Add(optionalBorderButton = newd BrushButton(this, g_gui.optional_brush, render_size, PALETTE_TERRAIN_OPTIONAL_BORDER_TOOL));
	optionalBorderButton->SetToolTip("Optional Border Tool");

	ASSERT(g_gui.eraser);
	size_sizer->Add(eraserButton = newd BrushButton(this, g_gui.eraser, render_size, PALETTE_TERRAIN_ERASER));
	eraserButton->SetToolTip("Eraser");

	ASSERT(g_gui.pz_brush);
	size_sizer->Add(pzBrushButton = newd BrushButton(this, g_gui.pz_brush, render_size, PALETTE_TERRAIN_PZ_TOOL));
	pzBrushButton->SetToolTip("PZ Tool");

	ASSERT(g_gui.rook_brush);
	size_sizer->Add(nopvpBrushButton = newd BrushButton(this, g_gui.rook_brush, render_size, PALETTE_TERRAIN_NOPVP_TOOL));
	nopvpBrushButton->SetToolTip("NO PVP Tool");

	ASSERT(g_gui.nolog_brush);
	size_sizer->Add(nologBrushButton = newd BrushButton(this, g_gui.nolog_brush, render_size, PALETTE_TERRAIN_NOLOGOUT_TOOL));
	nologBrushButton->SetToolTip("No Logout Tool");

	ASSERT(g_gui.pvp_brush);
	size_sizer->Add(pvpzoneBrushButton = newd BrushButton(this, g_gui.pvp_brush, render_size, PALETTE_TERRAIN_PVPZONE_TOOL));
	pvpzoneBrushButton->SetToolTip("PVP Zone Tool");

	ASSERT(g_gui.prefab_creator_brush);
	size_sizer->Add(prefabCreatorButton = newd BrushButton(this, g_gui.prefab_creator_brush, render_size, PALETTE_TERRAIN_PREFAB_CREATOR_TOOL));
	prefabCreatorButton->SetToolTip("Prefab Creator Tool (Click again to save/export)");

	ASSERT(g_gui.normal_door_brush);
	size_sizer->Add(normalDoorButton = newd BrushButton(this, g_gui.normal_door_brush, render_size, PALETTE_TERRAIN_NORMAL_DOOR));
	normalDoorButton->SetToolTip("Normal Door Tool");

	ASSERT(g_gui.locked_door_brush);
	size_sizer->Add(lockedDoorButton = newd BrushButton(this, g_gui.locked_door_brush, render_size, PALETTE_TERRAIN_LOCKED_DOOR));
	lockedDoorButton->SetToolTip("Locked Door Tool");

	ASSERT(g_gui.magic_door_brush);
	size_sizer->Add(magicDoorButton = newd BrushButton(this, g_gui.magic_door_brush, render_size, PALETTE_TERRAIN_MAGIC_DOOR));
	magicDoorButton->SetToolTip("Magic Door Tool");

	ASSERT(g_gui.quest_door_brush);
	size_sizer->Add(questDoorButton = newd BrushButton(this, g_gui.quest_door_brush, render_size, PALETTE_TERRAIN_QUEST_DOOR));
	questDoorButton->SetToolTip("Quest Door Tool");

	ASSERT(g_gui.hatch_door_brush);
	size_sizer->Add(hatchDoorButton = newd BrushButton(this, g_gui.hatch_door_brush, render_size, PALETTE_TERRAIN_HATCH_DOOR));
	...
	size_sizer->Add(checkbox_sub_sizer);
...
wxSize BrushToolPanel::GetMinSize() const {
	if (!loaded) return wxPanel::GetMinSize();
	
	int dock_dir = 0;
	if (g_gui.GetAuiManager()) {
		wxAuiPaneInfo& pane = g_gui.GetAuiManager()->GetPane(const_cast<BrushToolPanel*>(this));
		if (pane.IsOk() && !pane.IsFloating()) {
			dock_dir = pane.dock_direction;
		}
	}
	...
	int btn_size = large_icons ? 32 : 16;
	if (dock_dir == wxAUI_DOCK_TOP || dock_dir == wxAUI_DOCK_BOTTOM) {
		int min_w = 15 * btn_size + (large_icons ? 42 : 24) + 120;
		int min_h = btn_size + 12;
		return wxSize(min_w, min_h);
	} else {
		int min_w = 3 * btn_size + 16;
		int min_h = 6 * btn_size + 24;
		return wxSize(min_w, min_h);
	}
}
...
void BrushToolPanel::DeselectAll() {
	if (loaded) {
		...
		prefabCreatorButton->SetValue(false);
	}
}
...
Brush* BrushToolPanel::GetSelectedBrush() const {
	...
	if (prefabCreatorButton->GetValue()) {
		return g_gui.prefab_creator_brush;
	}
	return nullptr;
}
...
bool BrushToolPanel::SelectBrush(const Brush* whatbrush) {
	...
	} else if (whatbrush == g_gui.prefab_creator_brush) {
		button = prefabCreatorButton;
	}
...
void BrushToolPanel::OnClickPrefabCreatorButton(wxCommandEvent& event) {
	g_gui.ActivatePalette(GetParentPalette());
	if (g_gui.GetCurrentBrush() == g_gui.prefab_creator_brush) {
		g_gui.SavePrefabFromCreator();
	} else {
