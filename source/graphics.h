...
public:
	...
	// SVG icon support
	NSVGimage* loadSVG(const std::string& filename);
	void renderSVG(NVGcontext* vg, NSVGimage* img, float x, float y, float w, float h);

	void generateEmissiveLUT(uint32_t& textureId);
