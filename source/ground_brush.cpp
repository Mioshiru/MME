...
bool AutoBorder::load(pugi::xml_node node, wxArrayString &warnings,
                      GroundBrush *owner, uint16_t ground_equivalent) {
  ...
  for (pugi::xml_node childNode = node.first_child(); childNode;
    ...
    if (it.id == 0) {
      warnings.push_back("Invalid item ID " + std::to_string(itemid) +
                         " for border " + std::to_string(id));
      continue;
    }
    ...
    it.alwaysOnBottom = true; // Never-ever place other items under this, will
                              // confuse the user something awful.
    it.isBorder = true;
  ...
  for (pugi::xml_node childNode = node.first_child(); childNode;
    ...
    if (childName == "item") {
      ...
      if (it.brush && it.brush != this) {
        warnings.push_back("\nItem " + std::to_string(itemId) +
                           " can not be member of two brushes");
        return false;
      }
      ...
      if ((attribute = childNode.attribute("ground_equivalent"))) {
        ...
        if (it.id == 0) {
          warnings.push_back(
              "Invalid id of ground dependency equivalent item.\n");
          continue;
        } else if (!it.isGroundTile()) {
          warnings.push_back(
              "Ground dependency equivalent is not a ground item.\n");
          continue;
        } else if (it.brush && it.brush != this) {
          warnings.push_back("Ground dependency equivalent does not use the "
                             "same brush as ground border.\n");
          continue;
        }
      ...
      for (pugi::xml_node subChildNode = childNode.first_child(); subChildNode;
           subChildNode = subChildNode.next_sibling()) {
        if (as_lower_str(subChildNode.name()) != "specific") {
          continue;
        }

        SpecificCaseBlock *specificCaseBlock = nullptr;
        for (pugi::xml_node superChildNode = subChildNode.first_child();
             superChildNode; superChildNode = superChildNode.next_sibling()) {
          const std::string &superChildName =
              as_lower_str(superChildNode.name());
          if (superChildName == "conditions") {
            for (pugi::xml_node conditionChild = superChildNode.first_child();
                 conditionChild;
                 conditionChild = conditionChild.next_sibling()) {
              const std::string &conditionName =
                  as_lower_str(conditionChild.name());
              if (conditionName == "match_border") {
                ...
                int32_t edge_id =
                    AutoBorder::edgeNameToID(attribute.as_string());
                auto it = g_brushes.borders.find(border_id);
                if (it == g_brushes.borders.end()) {
                  warnings.push_back(
                      "Unknown border id in specific case match block " +
                      std::to_string(border_id));
                  continue;
                }
                ...
                specificCaseBlock->group_match_alignment =
                    ::BorderType(edge_id);
            ...
            for (pugi::xml_node actionChild = superChildNode.first_child();
      ...
      for (std::vector<BorderBlock *>::iterator it = borders.begin();
        ...
        BorderBlock *bb = *it;
        if (bb->autoborder) {
          for (std::vector<SpecificCaseBlock *>::iterator specific_iter =
                   bb->specific_cases.begin();
               specific_iter != bb->specific_cases.end(); ++specific_iter) {
...
void GroundBrush::draw(BaseMap *map, Tile *tile, void *parameter) {
  ...
  if (parameter != nullptr) {
    std::pair<bool, GroundBrush *> &param =
        *reinterpret_cast<std::pair<bool, GroundBrush *> *>(parameter);
...
const GroundBrush::BorderBlock *GroundBrush::getBrushTo(GroundBrush *first,
                                                        GroundBrush *second) {
  // printf("Border from %s to %s : ", first->getName().c_str(),
    ...
    if (second) {
      ...
      if (first->getZ() < second->getZ() && second->hasOuterBorder()) {
        if (first->hasInnerBorder()) {
          for (std::vector<BorderBlock *>::iterator it = first->borders.begin();
               it != first->borders.end(); ++it) {
            BorderBlock *bb = *it;
            if (bb->outer) {
              continue;
        ...
        for (std::vector<BorderBlock *>::iterator it = second->borders.begin();
             it != second->borders.end(); ++it) {
          BorderBlock *bb = *it;
          if (!bb->outer) {
            continue;
...
inline GroundBrush *extractGroundBrushFromTile(BaseMap *map, uint32_t x,
...
void GroundBrush::doBorders(BaseMap *map, Tile *tile) {
  static const auto extractGroundBrushFromTile =
      [](BaseMap *map, uint32_t x, uint32_t y, uint32_t z) -> GroundBrush * {
  ...
  for (int32_t i = 0; i < 8; ++i) {
    ...
    if (borderBrush) {
      if (other) {
        ...
        if (other->hasOuterBorder() || borderBrush->hasInnerBorder()) {
          ...
          if (/*!borderBrush->hasInnerBorder() && */ (
              other->friendOf(borderBrush) || borderBrush->friendOf(other))) {
          ...
          for (int32_t j = i; j < 8; ++j) {
            ...
            if (!otherPair.first && otherPair.second &&
                otherPair.second->getID() == other->getID()) {
                ...
                for (BorderCluster &borderCluster : borderList) {
                  if (borderCluster.border == borderBlock->autoborder) {
                    ...
                    if (!borderBlock->specific_cases.empty()) {
                      if (std::find(specificList.begin(), specificList.end(),
                                    borderBlock) == specificList.end()) {
    ...
    } else if (other && other->hasOuterZilchBorder() && !tile->hasGround()) {
      // Border against true void/empty space (never over an existing floor/ground item)
      uint32_t tiledata = 0;
      for (int32_t j = i; j < 8; ++j) {
        auto &otherPair = neighbours[j];
        if (!otherPair.first && otherPair.second &&
  ...
  while (!borderList.empty()) {
    ...
    BorderType directions[4] = {
        static_cast<BorderType>(
            (border_types[borderCluster.alignment] & 0x000000FF) >> 0),
        static_cast<BorderType>(
            (border_types[borderCluster.alignment] & 0x0000FF00) >> 8),
        static_cast<BorderType>(
            (border_types[borderCluster.alignment] & 0x00FF0000) >> 16),
        static_cast<BorderType>(
            (border_types[borderCluster.alignment] & 0xFF000000) >> 24)};
  ...
  for (const BorderBlock *borderBlock : specificList) {
    for (const SpecificCaseBlock *specificCaseBlock :
         borderBlock->specific_cases) {
      /*
      printf("New round\n");
      ...
      */
      ...
      for (Item *item : tile->items) {
        ...
        if (specificCaseBlock->match_group > 0) {
          ...
          // specificCaseBlock->match_group, item->getBorderAlignment(),
          // specificCaseBlock->group_match_alignment);
          if (item->getBorderGroup() == specificCaseBlock->match_group &&
              item->getBorderAlignment() ==
                  specificCaseBlock->group_match_alignment) {
            // printf("Successfully matched %d == %d : %d == %d\n",
            // item->getBorderGroup(), specificCaseBlock->match_group,
            // item->getBorderAlignment(),
            // specificCaseBlock->group_match_alignment);
            ++matches;
            continue;
