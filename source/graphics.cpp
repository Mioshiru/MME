...
NSVGimage* GraphicManager::loadSVG(const std::string& filename) {
    // Sicherheitscheck für Pfade (wie in lua_api_image implementiert)
    if (filename.find("..") != std::string::npos) return nullptr;

    NSVGimage* image = nsvgParseFromFile(filename.c_str(), "px", 96.0f);
    if (!image) {
        wxLogDebug("Failed to parse SVG: %s", filename);
        return nullptr;
    }
    return image;
}

void GraphicManager::renderSVG(NVGcontext* vg, NSVGimage* img, float x, float y, float w, float h) {
    if (!vg || !img) return;

    nvgSave(vg);
    
    // Skaliere SVG auf Zielgröße
    float scale = std::min(w / img->width, h / img->height);
    nvgTranslate(vg, x, y);
    nvgScale(vg, scale, scale);

    // Iteriere durch SVG-Pfade und zeichne mit NanoVG
    for (NSVGshape* shape = img->shapes; shape != NULL; shape = shape->next) {
        ...
        for (NSVGpath* path = shape->paths; path != NULL; path = path->next) {
            ...
            if (shape->stroke.type == NSVG_PAINT_COLOR) {
                ...
                nvgStrokeColor(vg, nvgRGBA(c & 0xff, (c >> 8) & 0xff, (c >> 16) & 0xff, (c >> 24) & 0xff));
...
bool GraphicManager::loadEditorSprites() {
	...
	{
		wxBitmap* prefab_b16x16 = nullptr;
		wxBitmap* prefab_b32x32 = nullptr;
		wxImage prefab_img;
		wxString prefab_path1 = "icons/prefab.png";
		wxString prefab_path2 = "../icons/prefab.png";
		wxString prefab_path3 = "Map Editor/icons/prefab.png";
		wxString prefab_path4 = "../Map Editor/icons/prefab.png";
		wxString prefab_path_to_load = "";
		if (wxFileExists(prefab_path1)) prefab_path_to_load = prefab_path1;
		else if (wxFileExists(prefab_path2)) prefab_path_to_load = prefab_path2;
		else if (wxFileExists(prefab_path3)) prefab_path_to_load = prefab_path3;
		else if (wxFileExists(prefab_path4)) prefab_path_to_load = prefab_path4;

		if (!prefab_path_to_load.IsEmpty() && prefab_img.LoadFile(prefab_path_to_load, wxBITMAP_TYPE_PNG)) {
			prefab_b32x32 = newd wxBitmap(prefab_img.Scale(32, 32));
			prefab_b16x16 = newd wxBitmap(prefab_img.Scale(16, 16));
		} else {
			prefab_b32x32 = loadPNGFile(eraser_png);
			prefab_b16x16 = loadPNGFile(eraser_small_png);
		}
		sprite_space[EDITOR_SPRITE_PREFAB] = newd EditorSprite(prefab_b16x16, prefab_b32x32);
	}
...
bool GraphicManager::loadSpriteMetadata(const FileName& datafile, wxString& error, wxArrayString& warnings) {
	...
	while (id <= maxID) {
		...
		if (!loadSpriteMetadataFlags(file, sType, error, warnings)) {
            std::string msg = std::format("Failed to load flags for sprite {}", sType->id);
			warnings.push_back(wxstr(msg));
		}
...
bool GraphicManager::loadSpriteMetadataFlags(FileReadHandle& file, GameSprite* sType, wxString& error, wxArrayString& warnings) {
	...
	for (int i = 0; i < DatFlagLast; ++i) {
		...
		switch (flag) {
			...
			default: {
                std::string err = std::format("Metadata: Unknown flag: {}. Previous flag: {}.", (int)flag, (int)prev_flag);
				warnings.push_back(wxstr(err));
				break;
			}
		}
	}
...
void GraphicManager::generateEmissiveLUT(uint32_t& textureId) {
    // Erstelle einen Buffer für 4096 mögliche Items (RGBA float)
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, 4096, 1, 0, GL_RGBA, GL_FLOAT, lutData.data());
...
#define safe_get(func, ...)                      \
	...
	for (std::vector<uint32_t>::iterator sprite_iter = sprite_indexes.begin(); sprite_iter != sprite_indexes.end(); ++sprite_iter, ++id) {
		...
		if (it != image_space.end()) {
			...
			if (spr && size > 0) {
				if (spr->size > 0) {
                    std::string ss = std::format("items.spr: Duplicate GameSprite id {}", id);
					warnings.push_back(wxstr(ss));
