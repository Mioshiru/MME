		...
		if (node_type == OTBM_TILE_AREA) {
			...
			for (BinaryNode* tileNode = mapNode->getChild(); tileNode != nullptr; tileNode = tileNode->advance()) {
				...
				if (tile_type == OTBM_TILE || tile_type == OTBM_HOUSETILE) {
					...
					tile = map.allocator(map.createTileL(pos.x, pos.y, pos.z));
					House* house = nullptr;
			...
			for (BinaryNode* waypointNode = mapNode->getChild(); waypointNode != nullptr; waypointNode = waypointNode->advance()) {
				...
				map.waypoints.addWaypoint(newd Waypoint(wp.name, wp.pos));
			}
		}
	}
...
bool IOMapOTBM::loadWaypoints(Map& map, pugi::xml_document& doc) {
	...
	for (pugi::xml_node wpNode = node.first_child(); wpNode; wpNode = wpNode.next_sibling()) {
		if (as_lower_str(wpNode.name()) != "waypoint") continue;
		...
		if (pos.isValid() && !name.empty()) {
			map.waypoints.addWaypoint(newd Waypoint(name, pos));
		}
	}
	return true;
};
...
bool IOMapOTBM::saveMap(Map& map, const FileName& identifier) {
	...
	// Optimierung: Erst in Speicher schreiben, dann als Ganzes auf Disk
	MemoryNodeFileWriteHandle f;
	if (!saveMap(map, f)) {
		return false;
	}
