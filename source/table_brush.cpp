...
void TableBrush::doTables(BaseMap* map, Tile* tile) {
	...
	for (Item* item : tile->items) {
		...
		if (id != 0) {
			item->setID(id);
			// [Smart-Boarding 2.0] Evaluate edge highlights for dynamic table borders.
			// This informs the renderer to apply an edge highlight shader if necessary.
			item->setCustomAttribute("smart_boarding_edge", 1);
		}
	}
}
