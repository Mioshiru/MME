...
void Waypoints::addWaypoint(Waypoint* wp) {
	...
	if (wp->pos != Position()) {
		...
		t->getLocation()->increaseWaypointCount(); // qtree_node ist jetzt public
		if (t->getLocation() && t->getLocation()->qtree_node) { 
			t->getLocation()->qtree_node->markDirty(t->getLocation()->getZ());
		}
	}
