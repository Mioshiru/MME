...
public:
	...
	QTreeNode* getLeaf(int x, int y) const {
		uint32_t lx = uint32_t(x) >> 2;
		uint32_t ly = uint32_t(y) >> 2;
		if (lx == last_leaf_x && ly == last_leaf_y && last_leaf != nullptr) {
			return last_leaf;
		}
		QTreeNode* leaf = const_cast<QTreeNode&>(root).getLeaf(x, y);
		if (leaf) {
			last_leaf_x = lx;
			last_leaf_y = ly;
			last_leaf = leaf;
	...
	void clearLeafCache() {
		last_leaf_x = 0xFFFFFFFF;
		last_leaf_y = 0xFFFFFFFF;
		last_leaf = nullptr;
	}
...
protected:
	...
	mutable uint32_t last_leaf_x = 0xFFFFFFFF;
	mutable uint32_t last_leaf_y = 0xFFFFFFFF;
...
inline uint64_t BaseMap::getTileCount() const {
	return tilecount;
}

#endif // RME_BASE_MAP_H_
