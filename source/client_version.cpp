...
bool ClientVersion::hasValidPaths() {
	if (!client_path.DirExists()) {
		FileName defaultPath = getDataPath();
		if (defaultPath.DirExists()) {
			client_path = defaultPath;
		}
		// Fallback to data_path if Tibia.dat is placed there instead of an explicitly configured client_path
		if (!wxFileName(client_path.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + "Tibia.dat").FileExists()) {
			FileName dataPathFallback = getDataPath();
			if (wxFileName(dataPathFallback.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + "Tibia.dat").FileExists()) {
				client_path = dataPathFallback;
			}
		}
	}
	if (!client_path.DirExists()) {
