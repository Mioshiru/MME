...
void Brushes::init() {
	...
	addBrush(g_gui.prefab_creator_brush = newd PrefabCreatorBrush());
...
//=============================================================================
// Prefab Brush - Zeichnet gespeicherte Snapshots
...
void PrefabBrush::draw(BaseMap* map, Tile* tile, void* parameter) {
    ...
    Map* realMap = dynamic_cast<Map*>(map);
    if (realMap) {
        Position pos = tile->getPosition();
        // Nutze die neue Snapshot-Wiederherstellung (aus map.cpp)
        auto decoded = LuaAPI::base64_decode(m_base64_data);
        realMap->restoreRegionSnapshot(pos.x, pos.y, decoded);
    }
}

void PrefabBrush::undraw(BaseMap* map, Tile* tile) {
    // Prefabs löscht man am besten mit dem Eraser, 
    // da sie aus vielen verschiedenen Brushes bestehen können.
}

std::string PrefabBrush::getName() const {
    return m_name;
}

int PrefabBrush::getLookID() const {
    // Wenn kein Sprite gesetzt ist, nutzen wir das prefab.png Icon (EDITOR_SPRITE_PREFAB)
    return look_id != 0 ? look_id : EDITOR_SPRITE_PREFAB;
}

PrefabCreatorBrush::PrefabCreatorBrush() : selected_tiles() {
}

PrefabCreatorBrush::~PrefabCreatorBrush() {
	clear();
}

void PrefabCreatorBrush::draw(BaseMap* map, Tile* tile, void* parameter) {
	if (!tile) return;
	selected_tiles.insert(tile->getPosition());
}

void PrefabCreatorBrush::undraw(BaseMap* map, Tile* tile) {
	if (!tile) return;
	selected_tiles.erase(tile->getPosition());
}

void PrefabCreatorBrush::markPosition(const Position& pos, bool select) {
	if (select) {
		selected_tiles.insert(pos);
	} else {
