...
TileLocation::TileLocation() :
	tile(nullptr), // Standardkonstruktor, qtree_node wird später gesetzt
	position(0, 0, 0),
	spawn_count(0),
	waypoint_count(0),
	town_count(0),
	house_exits(nullptr), // Initialisierung von qtree_node erfolgt im Floor-Konstruktor
	qtree_node(nullptr)
{
}
...
Floor::Floor(QTreeNode* parent_node, int sx, int sy, int z) {
	sx = sx & ~3;
	sy = sy & ~3;

	for (int i = 0; i < 16; ++i) { // 4x4 Tiles
		locs[i].qtree_node = parent_node; // Setze den QTreeNode-Pointer hier
...
Floor::~Floor() {
    if (vbo_id != 0) {
        // Die Löschung erfolgt im Render-Thread via GraphicManager (Cleanup-Queue)
    }
}
...
QTreeNode::QTreeNode(BaseMap& map) :
	map(map),
	visible(0),
	dirty_flags(0), // Dirty-Flags initialisieren
	isLeaf(false) {
...
void QTreeNode::clearAllDirty(int z) {
	dirty_flags = 0;
	if (!isLeaf) {
		for (int i = 0; i < MAP_LAYERS; ++i) { if (child[i]) child[i]->clearAllDirty(z); }
	}
}

Floor* QTreeNode::createFloor(BaseMap* map, int x, int y, int z) {
	ASSERT(isLeaf);
	if (!array[z]) {
		array[z] = newd Floor(this, x, y, z); // 'this' als parent_node übergeben
	}	
	return array[z];
}
